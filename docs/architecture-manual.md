...
- Ingest、基础 QA Agent Loop、Checkpoint 中断续跑、用户可读 Session 文件、约 60% 窗口阈值的动态上下文压缩、资源哈希失效重读、文件型项目级长期记忆和只读用户 Profile 已具备可运行实现；Answer Review 尚未完成。
...
6. 合法答案先把 `memory_patch` 幂等合并到最新项目记忆，再原子保存完整 Session 消息日志、合并后的记忆、资源元数据和本轮使用的压缩边界，最后写入运行结果并结束。非法 JSON、未知工具、本轮重复调用 ID、伪造引用或模型步数耗尽进入统一失败终态。
...
| `src/paperscout/qa.py` | 定义并运行 QA `StateGraph`：严格 JSON 模型动作、工具循环、审计、预算、结构化回答、Evidence 校验、项目/Session 记忆、多轮继续、上下文压缩、资源失效提示和 `resume_qa()`。 | P0-C2、P0-D1 和 P0-D2 已完成；QA 窗口约 60% 的动态压缩使用保守 token 估算，Answer Review 后续实现。 |
...
离线 pytest 证明当前自动测试覆盖的行为没有回归，但 Mock QA/Review 只验证控制流和契约，不能证明真实问答或语义审核质量；保守 token 估算也不等同于具体 Provider tokenizer 的精确计数。尚未实现的 Answer Review 不能因基线通过而标记完成。真实 MinerU/LLM 只通过对应人工脚本单独验证，不能混入离线基线结论。
