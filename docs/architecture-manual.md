...
- Ingest、基础 QA Agent Loop、Checkpoint 中断续跑、用户可读 Session 文件、最近四轮压缩、资源哈希失效重读、文件型项目级长期记忆和只读用户 Profile 已具备可运行实现；动态 token 压缩和 Answer Review 尚未完成。
...
模型上下文加载只读用户 Profile、当前项目长期记忆、Session 摘要、最近四个已完成交互轮次和当前问题。更早轮次被确定性压缩到结构化 `summary.md`：保留项目记忆、有效资源的路径、offset、字符数、SHA256、Evidence ID，以及每个旧轮次的精简用户问题和最终回答；旧工具正文仍完整留在 `messages.jsonl`，但不会回流模型。摘要最多列出最近 20 个已压缩轮次，更老轮次只保留在完整审计中。压缩会记录 `context.compacted` 事件。当前采用固定四轮边界，尚未实现按真实模型 token 窗口约 60% 动态触发。
...
1. `run_qa()` 加载只读用户 Profile，校验 `project_id` 与 `session_id`，加载项目长期记忆和绑定同一项目的用户可读 Session，建立独立运行与 `thread_id`，将 Profile 快照、项目 ID、结构化摘要、最近四轮、合并记忆和当前用户问题写入 `QAGraphState` 并启动 Checkpointed `StateGraph`。
...
| `tests/test_qa_graph.py` | 验证精简工具结果、严格模型 JSON、渐进读取、引用失败关闭、节点中断/瞬时故障恢复、模型/工具持久化重放、预算不重复计费、终态幂等 resume、Session 三文件、多轮继续、路径保护、写后故障幂等恢复、固定四轮压缩、资源哈希失效重读、项目记忆共享/隔离/恢复，以及用户 Profile 跨项目只读加载。 | 是。 |
...
离线 pytest 证明当前自动测试覆盖的行为没有回归，但 Mock QA/Review 只验证控制流和契约，不能证明真实问答或语义审核质量；也不能证明尚未实现的动态 token 预算或 Answer Review 已完成。真实 MinerU/LLM 只通过对应人工脚本单独验证，不能混入离线基线结论。
