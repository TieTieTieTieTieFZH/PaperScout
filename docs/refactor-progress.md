...
| --- | --- | --- | --- |
| 离线测试集 | 85 tests | 本地自动验证 | `.venv\Scripts\python.exe -m pytest -q`；越界 reparse point 在普通 symlink 不可用时通过 Windows junction 回退实测 |
...
离线测试只证明表中已覆盖的控制流和契约；Mock QA/Review 不能证明真实语义质量，也不证明尚未实现的动态 token 阈值、全局用户 Profile 或 Answer Review。
...
| P0-A Session/Graph State/Tool/Event 契约 | 仅契约 | 严格项目状态、Session、预算、Tool Call、Graph State、WorkflowEvent 模型。 | 本地自动验证 | `tests/test_p0a_contracts.py`、`tests/test_evidence_contract.py` | 执行层分别属于 P0-C、P0-D、P1。 |
...
| P0-C2 QA Agent Loop | 已完成 | 真实 QA `StateGraph`、严格单对象 JSON 动作、单工具条件循环、精简模型可见结果、完整宿主审计、错误纠正、预算耗尽处理、Wiki→Evidence→raw 渐进读取、结构化回答及论文/Evidence 归属校验。 | 本地自动验证 | `tests/test_qa_graph.py`：23 passed；覆盖 Mock 公开入口、四级渐进读取、错误修正、预算、跨论文与伪造引用 | 真实 LLM 问答质量未人工验证；Answer Review 属于后续阶段。 |
...
| P0-D2 Session、压缩与记忆 | 部分实现 | 用户可读 Session 三文件原子写入；安全 Session/项目路径；可选 `project_id="default"`；`memory/projects/{project_id}/state.json` 项目记忆；跨 Session 共享与跨项目隔离；Session 项目绑定；记忆/资源合并去重；模型只加载最近四轮与当前问题；更早轮次写入结构化摘要；旧工具正文不回流模型；加载时重新核对资源哈希并强制重新读取；`context.compacted` 事件；项目/Session 写后故障从 `complete_qa` 幂等续跑且不重复消息或模型调用。 | 本地自动验证 | `tests/test_qa_graph.py`：三文件、多轮继续、路径保护、写后故障恢复、固定四轮压缩、资源失效，以及项目记忆共享/隔离/默认兼容/项目绑定/恢复测试 | 真实模型 token 窗口约 60% 的动态触发和全局用户 `profile.json` 尚未实现。 |
...
| 2026-09-08 | （本提交） | 完成 P0-D2 可选 `project_id="default"`、文件型项目状态、跨 Session 共享、项目隔离、Session 项目绑定和写后故障幂等恢复。 | QA 专项 23 passed；完整离线基线 85 passed；依赖锁与补丁格式检查通过。 |

## 5. 项目经历表述验收

| 简历候选表述 | 当前是否可以写成“已实现” | 证据与原因 |
| --- | --- | --- |
| 自研领域 Agent Harness | 可以，限定为本地 PaperScout Harness | Agent Loop、工具生命周期、Session 文件、固定最近四轮压缩、文件型项目级长期记忆、Checkpointer 和中断/瞬时故障恢复均已落地并有自动测试；动态 token 阈值和全局用户 Profile 尚未实现，不应扩大表述。 |
...
下一最小切片是 P0-D2 的全局用户 `memory/profile.json`：只保存跨项目稳定的回答风格和引用偏好，并继续保持与项目记忆、Session、Checkpoint、审计事件隔离；随后再接入真实模型 token 窗口约 60% 的动态压缩阈值。
