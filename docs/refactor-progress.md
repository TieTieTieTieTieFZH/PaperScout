...
| --- | --- | --- | --- |
| 离线测试集 | 95 tests | 本地自动验证 | `.venv\Scripts\python.exe -m pytest -q`；越界 reparse point 在普通 symlink 不可用时通过 Windows junction 回退实测 |
...
离线测试只证明表中已覆盖的控制流和契约；Mock QA/Review 不能证明真实语义质量，保守 token 估算不等同于具体 Provider tokenizer 的精确计数，也不证明尚未实现的语义 Answer Review。
...
| P0-C2 QA Agent Loop | 已完成 | 真实 QA `StateGraph`、严格单对象 JSON 动作、单工具条件循环、精简模型可见结果、完整宿主审计、错误纠正、预算耗尽处理、Wiki→Evidence→raw 渐进读取、结构化回答及论文/Evidence 归属校验。 | 本地自动验证 | `tests/test_qa_graph.py`：28 passed；覆盖 Mock 公开入口、四级渐进读取、错误修正、预算、跨论文与伪造引用 | 真实 LLM 问答质量未人工验证；语义 Answer Review 属于后续切片。 |
...
| P0-E Answer Review | 部分实现 | 显式 `answer_rules` 与返回前 `verify_answer_evidence` 节点；复用严格 QA Claim/引用规则；只安全收集候选实际引用的当前 section Evidence；校验文件内 ID；显式原文/依据请求不得无引用回答；审核后 Evidence 哈希变化失败关闭；规则结论写入 Graph State、结果和事件。 | 本地自动验证 | `tests/test_answer_review.py`：4 passed；`tests/test_qa_graph.py`：Evidence 在规则后变化时不保存 Session | 独立语义 Answer Review Chat Client、审核 request/response/result、`REVISE/REJECT` 补读/修订循环与最大次数安全降级尚未实现。 |
| P1 事件与可观测性 | 部分实现 | Ingest 与 QA 严格 JSONL 运行/模型、中断、恢复及终态事件；QA 工具、上下文压缩及确定性 Answer Review 开始/完成事件；压缩事件携带窗口、阈值、估算 token 和保留轮数；恢复后序号连续。 | 本地自动验证 | Ingest/QA 恢复事件序列、动态压缩和 Answer 规则事件测试 | 语义 Answer Review 事件、流式消费和回放未实现；节点重试事件语义当前为 at-least-once。 |
...
| 2026-09-08 | （本提交） | 完成 P0-E 确定性 Answer Review 规则、实际引用 Evidence 收集契约和返回前哈希复核。 | Answer Review 专项 4 passed；Answer Review、QA 与契约专项 38 passed；完整离线基线 95 passed；依赖锁与补丁格式检查通过。 |
...
| 基于 LangGraph 编排 Ingest、QA、Review | 否 | Ingest、QA 与 Wiki Review 已由 LangGraph 编排，Answer Review 只有确定性规则层，语义审核尚未实现，仍不能扩大表述为完整多 Agent 编排。 |
...
| 规则校验 + Review Agent 双阶段审核 | 只能限定为 Wiki Ingest | Wiki 的确定性规则与语义 Review 已接入；Answer Review 只有确定性规则层，语义 Review 尚未实现，因此不能描述为全系统双审核。 |
| 审核驱动自动修订或发布 | 可以，限定为 Wiki Ingest | Wiki Review verdict 已实际控制完整重生成、失败关闭与发布；QA 答案尚不具备该能力。 |

## 6. 下一步

下一最小切片是 P0-E 语义 Answer Review：接入与 QA Provider 隔离、无工具无 Session 的 Chat Client，保存逐次 request/response/result，并先完成 `APPROVE` 放行与非法 verdict 失败关闭；随后接入 `REVISE/REJECT` 补读/修订循环和最大次数安全降级。
