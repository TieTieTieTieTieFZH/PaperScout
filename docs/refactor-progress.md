...
| --- | --- | --- | --- |
| 离线测试集 | 100 tests | 本地自动验证 | `.venv\Scripts\python.exe -m pytest -q`；越界 reparse point 在普通 symlink 不可用时通过 Windows junction 回退实测 |
...
离线测试只证明表中已覆盖的控制流和契约；Mock QA/Review 不能证明真实语义质量，保守 token 估算不等同于具体 Provider tokenizer 的精确计数，也不证明尚未实现的 Answer Review 修订循环。
...
| P0-C2 QA Agent Loop | 已完成 | 真实 QA `StateGraph`、严格单对象 JSON 动作、单工具条件循环、精简模型可见结果、完整宿主审计、错误纠正、预算耗尽处理、Wiki→Evidence→raw 渐进读取、结构化回答及论文/Evidence 归属校验。 | 本地自动验证 | `tests/test_qa_graph.py`：33 passed；覆盖 Mock 公开入口、四级渐进读取、错误修正、预算、跨论文与伪造引用 | 真实 LLM 问答质量未人工验证。 |
...
| P0-E Answer Review | 部分实现 | 显式 `answer_rules`、独立 `answer_review` 与返回前 `verify_answer_evidence` 节点；只安全收集候选实际引用的当前 section Evidence；确定性规则通过后，以独立、无工具、无 Session Provider 审核问题、完整回答和实际引用 Evidence；严格解析首行 verdict；`APPROVE` 放行；非法输出及当前无法自动修订的 `REVISE/REJECT` 失败关闭；逐次保存 request/response/result；Provider 瞬时故障可续跑且不重复 QA；最终哈希变化不保存 Session。 | 本地自动验证 | `tests/test_answer_review.py`：4 passed；`tests/test_qa_graph.py`：语义审核专项 5 passed，QA 专项 33 passed | `REVISE/REJECT` 补读/修订/重新审核循环、最大次数后的受支持内容或证据不足安全降级尚未实现。 |
...
| 2026-09-08 | （本提交） | 完成 P0-E 独立语义 Answer Review `APPROVE` 审核门、逐次审计、严格 verdict 失败关闭和瞬时故障恢复。 | 语义 Answer Review 专项 5 passed；Answer Review/Review/QA 专项 44 passed；完整离线基线 100 passed；依赖锁与补丁格式检查通过。 |
...
| 基于 LangGraph 编排 Ingest、QA、Review | 只能限定表述 | Ingest、QA、Wiki Review 与 Answer Review 审核门均由 LangGraph 编排；Answer Review 自动修订循环尚未实现，不能表述为完整闭环。 |
...
| 规则校验 + Review Agent 双阶段审核 | 可以，限定审核门 | Wiki 与 QA 都已接入确定性规则和独立语义 Review；QA 的非通过结果当前只会失败关闭，尚不具备自动修订。 |
| 审核驱动自动修订或发布 | 可以，限定为 Wiki Ingest | Wiki Review verdict 已实际控制完整重生成、失败关闭与发布；QA 答案尚不具备该能力。 |

## 6. 下一步

下一最小切片是 P0-E Answer Review 修订循环：把 `REVISE/REJECT` 意见反馈给 QA，允许继续补读或重新回答，并对每个完整候选重新执行确定性与语义审核；达到最大次数后只返回受支持内容或明确报告证据不足。
